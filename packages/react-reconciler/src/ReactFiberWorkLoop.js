...
export function computeExpirationForFiber(
  ...
  //render 阶段rootFider的mode 为0， 那么返回的过期时间为Sync为最大
...
export function scheduleUpdateOnFiber(
  ...
  // 判断是否在componentWillUpdate 和 ComponentDidUpdate中调用setState， 判断是否无限循环
  ...
  // 判断是否有高优先级打断当前正在执行的任务
  checkForInterruption(fiber, expirationTime);
  recordScheduleUpdate();

  // TODO: computeExpirationForFiber also reads the priority. Pass the
  // priority as an argument to that function and this one.
  // 获取当前任务调度的优先级，默认是NormalPriority
  const priorityLevel = getCurrentPriorityLevel();

  if (expirationTime === Sync) {
    if (
      // Check if we're inside unbatchedUpdates
      // 未渲染未update是未分批次的， 也就是处于第一次渲染钱
...
// on a fiber.
// 更新子节点的expirationTime
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  // Update the source fiber's expiration time
  // 如果发生更新的expirationTime 大于原本的expirationTime, 那么重新赋值
  if (fiber.expirationTime < expirationTime) {
    fiber.expirationTime = expirationTime;
  }
  // 这个fiber.alternate和fiber.current是一样的， 这个alternate指向的节点，就是用新的props和state创建的节点
  ...
  if (node === null && fiber.tag === HostRoot) {
    // 这里面的node是rootFiber,然后调用stateNode属性会指向fiberRoot
    root = fiber.stateNode;
  } else {
    // 如果不是rootFiber
    while (node !== null) {
      alternate = node.alternate;
      // 如果父节点中的所有子节点中优先级最高的更新时间仍然小于expirationTime的话，那明天重新设置childExpirationTime的时间
...
function checkForInterruption(
  ...
  if (
    ...
    // 更新任务正在执行，且新来的任务的优先级大于当前更新的任务，那么需要打断当前任务
...
function scheduleInteractions(root, expirationTime, interactions) {
  ...
  if (interactions.size > 0) {
    ...
    // 如果不为null，遍历， 这个pendingInteractions是一个map结构
    if (pendingInteractions != null) {
      ...
      // 如果没有的话就初始化，并把当前调度中的任务存进pendingInteractionMap，然后并统计
